...
def read_test():
    test_dataframe = pd.read_csv('../data_frames/test_dataframe.csv', sep=',', names=['id'])
...
def read_validation():
    validation_dataframe = pd.read_csv('../data_frames/validation_dataframe.csv', sep=',', names=['id', 'class'])
    ...
    validation_images = np.array(validation_images).reshape((-1, image_size[0], image_size[1], 1))
...
def read_train():
    train_dataframe = pd.read_csv('../data_frames/train_dataframe.csv', sep=',', names=['id', 'class'])
...
def read_train_smote(slice_value: int = None):
    ...
    if slice is None:
        ...
        train_images_ids_0 = train_dataframe[train_dataframe['class'] == 0]['id'].values[:slice_value]
        train_images_labels_0 = train_dataframe[train_dataframe['class'] == 0]['class'].values[:slice_value]
        train_images_ids_1 = train_dataframe[train_dataframe['class'] == 1]['id'].values
        train_images_labels_1 = train_dataframe[train_dataframe['class'] == 1]['class'].values

        train_images_ids = np.concatenate((train_images_ids_0, train_images_ids_1))
        train_images_labels = np.concatenate((train_images_labels_0, train_images_labels_1))
    ...
    train_images_smote = train_images_smote.reshape((-1, image_size[0], image_size[1], 1))
