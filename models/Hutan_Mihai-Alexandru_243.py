...
def generate_numpy_array(image_path: str) -> np.ndarray:
    ...
    return np.asarray(image).astype(np.float64) / 255  # Transform image to numpy array and normalize values
...
def generate_numpy_arrays_for_train_and_validation():
    ...
    for image_id in range(number_of_read_images + 1, number_of_last_image + 1):
        image_path = data_dir + str(image_id).zfill(6) + '.png'  # zfill is used to add leading zeros to the image_id
